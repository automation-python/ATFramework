...
class _TestInfo(object):
    ...
    def __init__(self, test_result, test_method, outcome=SUCCESS, err=None, subTest=None):
        ...
        self.CurrentCaseName = test_method.CurrentCaseName
        if subTest:
...
class _XMLTestResult(_TextTestResult):
    ...
    def _save_output_data(self):
        ...
        try:
            ...
            # redict stdout to Var by jianglan
    ...
    def addSuccess(self, test):
        ...
        Var.Pass += 1
        self.imagedir = Var.CurrentCaseDir
    ...
    def _createCDATAsections(xmldoc, node, text):
        #don't ADD syserr in report by jianglan
        return
    ...
    def _report_testcase(test_result, xml_testsuite, xml_document):
        ...
        testcase.setAttribute(
            'name', test_result.CurrentCaseName
        )
        testcase.setAttribute('start_time', '%.3f' % test_result.start_time)
        testcase.setAttribute('time', '%.3f' % test_result.elapsed_time)

        result_info = test_result.stdout.split("&&&&&")
        message = []
        for msg in result_info:
            if msg != "" and not msg.endswith(".png"):
                message.append(msg)
        testcase.setAttribute('message', ''.join(message))
    ...
    def generate_reports(self, test_runner, start_time, stop_time):
        """
        ...
        if not outputHandledAsString:
            ...
            testsuite.setAttribute('start_time', str(start_time))
            timestamp = time.strftime("%Y-%m-%d %H:%M:%S", timeArray)
            testsuite.setAttribute('timestamp', timestamp)
            duration_time = stop_time - start_time
            testsuite.setAttribute('time', '%.3f' % duration_time)
